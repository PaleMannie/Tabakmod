...
public class KamelZigItem extends Item {

    public KamelZigItem(Properties pProperties) { super(pProperties); }
//////////////////////////////////////////////////EIGENE METHODEN///////////////////////////////////////////////////////
    ...
    void gibRauchStandardEffekte(Player player, ItemStack stack, int gepaffteZeit){
        ...
        player.addEffect(new MobEffectInstance(MobEffects.NAUSEA,getUseDuration(stack, player)-gepaffteZeit+68,2));
        player.addEffect(new MobEffectInstance(MobEffects.STRENGTH,(2*(24*(getUseDuration(stack,player)-gepaffteZeit)))/10,2));
        player.addEffect(new MobEffectInstance(MobEffects.RESISTANCE,(2*(24*(getUseDuration(stack,player)-gepaffteZeit)))/10,2));
        player.addEffect(new MobEffectInstance(MobEffects.SPEED,(2*(24*(getUseDuration(stack,player)-gepaffteZeit)))/10,2));
    ...
    void ueberzugsEffekte(Player player, Level level){
        ...
        if(level instanceof ServerLevel sevel){
            player.hurtServer(sevel, level.damageSources().source(ModDamageTypes.KAMEL_SCHADEN), 2f);
        }
            ...
            if (pLivingEntity instanceof Player pPlayer && (pRemainingUseDuration <= getUseDuration(pStack, pLivingEntity) - 15)) {
                ...
                if (pStack.getDamageValue() >= pStack.getMaxDamage()) {
                    ...
                    if(pLevel instanceof ServerLevel serverlevel) {
                        Projectile.spawnProjectileFromRotation(ZigKamelStummelEntity::new, serverlevel, ModItems.STUMMEL_KAMEL.get().getDefaultInstance(), pPlayer, 0f, 0.5f, 10f);
                    }
