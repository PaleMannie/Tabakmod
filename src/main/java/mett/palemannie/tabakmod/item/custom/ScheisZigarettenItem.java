...
import mett.palemannie.tabakmod.entity.custom.ZigScheiseStummelEntity;
...
//////////////////////////////////////////////////EIGENE METHODEN///////////////////////////////////////////////////////
    ...
    void exhaliere(Level level, Player player, int wieVielRauchMussSein){
        ...
        if (level instanceof ServerLevel slevel) {
            ...
            slevel.sendParticles(ParticleTypes.CAMPFIRE_COSY_SMOKE, MausPos.x, MausPos.y-0.2d, MausPos.z, 4, 0.15d, 0d, 0.15d,0.02d);
        }
    }
    ...
    void ueberzugsEffekte(Player player, Level level){
        ...
        player.playSound(ModSounds.SCHEISE_GERAUCHT.get(), 1f, (float) RandomSource.create().nextInt(8, 12)/10);
    }
        ...
        if(!pLivingEntity.isUnderWater()) {
            ...
            if (pLivingEntity instanceof Player pPlayer && (pRemainingUseDuration <= getUseDuration(pStack, pLivingEntity) - 12)) {
                ...
                if (pStack.getDamageValue() >= pStack.getMaxDamage()) {
                    ...
                    pLevel.playSound(null, pLivingEntity.getX(), pLivingEntity.getY(), pLivingEntity.getZ(), ModSounds.SCHEISE_GERAUCHT.get(), SoundSource.PLAYERS, 1f, (float)RandomSource.create().nextInt(8, 12)/10);
                    if(pLevel instanceof ServerLevel serverlevel) {
                        Projectile.spawnProjectileFromRotation(ZigScheiseStummelEntity::new, serverlevel, ModItems.STUMMEL_SCHEISE.get().getDefaultInstance(), pPlayer, 0f, 0.5f, 10f);
                    }
