...
public class ZigarrenItem extends Item {
    ...
    void ueberzugsEffekte(Player player, Level level){
        ...
        if(level instanceof ServerLevel sevel){
            player.hurtServer(sevel, level.damageSources().source(ModDamageTypes.ZIGRR_SCHADEN), 2f);
        }
    ...
    public boolean releaseUsing(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity, int pTimeCharged) {
        ...
        if(pLivingEntity instanceof Player pPlayer && (pTimeCharged <= getUseDuration(pStack, pLivingEntity) - 25)){
